...
function ClearResource(_This,Ctrl)
{
    $.ajax({
        type: "POST",
        url: _This + "/ClearResource",
        data: "{'Ctrl' : '" + Ctrl + "'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
...
class Grid
{
    ...
    constructor(Pagename,HeaderColumns,SearchesDat,Searchcolumns, gridname, PagePerItem,Width,Data,Label,Fx,Search,NewButton,EditButton,DeleteButton,Panel,WPanel,HPanel,SortFieldName,FullRowSelect,Multiselect,PK,Criteria,SelName)
    {
        This = Pagename;
        if (SortFieldName == '')
        {
     ...
     _Tables() 
    {
        ...
        {   
            ...
            if (this.NewButton.includes("button")) {
                ...
                html += "<td id='TdNewButton_" + this.gridname + "' style='width:200px;text-align:left;'>";
                html += " <div style='margin-top: 20px; margin-bottom: 20px;'  id='divNewButton" + this.gridname + "'><button style='border-radius:1px;font-size:13px;' class='btn btn-secondary' id='lnknew_" + this.gridname + "' class='lnk-download' onClick=NewGrid('" + this.gridname + "','" + this.Panel + "','" + this.WPanel + "','" + this.HPanel + "');>" + this.NewButton + "</button></div>";
                html += "</td>";
            }
        }
        if (this.Multiselect != "")
        ...
        for (i == 0; i < this.columns.length; i++) {
            ...
            if (command != 'H')
            {
                html += "<th id='" + this.gridname + "_" + c + "' style='cursor:pointer;color:white;font-size:TH SarabunPSK;font-weight:100;margin:5px;text-decoration: underline; width: " + this.Width[i] + ";'>";
                html += "<div></div>";
