            ...
            if (DtResult.Rows.Count != 0)
            {
                ...
                if (EditButton.Trim() != "" || DeleteButton.Trim() != "")
                {
                    ...
                    foreach (DataRow dr in DtResult.Rows)
                    {
                        ...
                        if (EditButton.Trim() != "")
                        {
                            CtrlButton += "<button class='btn btn-warning' onClick=\"EditGrid('" + Ctrl + "','" + Criterias + "','" + Panel + "','" + WPanel + "','" + HPanel + "');\" id='GvEdit" + Ctrl + '_' + dr[PK].ToString() + "'><i class=\"fa fa-pencil\" aria-hidden=\"true\"></i></button>&nbsp;";

                        }
                        if (DeleteButton.Trim() != "")
                        {

                            CtrlButton += "<button class='btn btn-danger' onClick=\"DeleteGrid('" + Ctrl + "','" + Criterias + "','" + Panel + "');\" id='GvDel" + Ctrl + '_' + dr[PK].ToString() + "'><i class=\"fa fa-trash\" aria-hidden=\"true\"></i></button>";
                        }
                        dr["Ctrl"] = CtrlButton;
