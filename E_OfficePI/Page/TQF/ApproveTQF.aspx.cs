...
{
    public partial class ApproveTQF : System.Web.UI.Page
    {
        ...
        public static string ChangeValidate(string json)
        {
            ...
            if (cn.Select("Select * from Sys_Master_ApproveTQF where isdelete = 0 and userid = '" + ClsEngine.FindValue(Dicts, "userid") + "'").Rows.Count == 0)
            {
                string id = ClsEngine.GenerateRunningId(Connectionstring, "Sys_Master_ApproveTQF", "id");
                //insert

                sqlcmd = " INSERT INTO [Sys_Master_ApproveTQF] ";
                sqlcmd += " ([id] ";
                sqlcmd += " ,[userid] ";
            ...
            {
                sqlcmd = "Update Sys_Master_ApproveTQF set ";
        ...
        [WebMethod]
        public static List<ClsApproveTQF> GetApproverTQF(string json)
        {
            //json += 'CbAPOrg:' + $('#CbAPOrg').val() + '|';
            List<ClsDict> Dicts = new List<ClsDict>();
            List<ClsApproveTQF> Objs = new List<ClsApproveTQF>();
            SqlConnector cn = new SqlConnector(Connectionstring, null);
            Dicts = ClsEngine.DeSerialized(json, ':', '|');
            ClsApproveTQF Obj;
            string sqlcmd = "";
            ...
            ArrayList Arrres = ClsEngine.Getsuborg(ref cn, ClsEngine.FindValue(Dicts, "CbAPOrg").ToString());
            sqlcmd = "  Select isnull(EP.InsRegular,'') as EV_InsRegular,isnull(EP.[InsTheory],'') as EV_InsTheory,isnull(EP.[InsExtra],'') as EV_InsExtra, isnull(EP.InsRegular,'') as AP_InsRegular,isnull(EP.[InsTheory],'') as AP_InsTheory,isnull(EP.[InsExtra],'') as AP_InsExtra,e.userid,e.empno,E.FirstnameTH,E.LastnameTH ,EV.userid as Validatoruserid,isnull(EV.FirstnameTH,'') + ' ' +  isnull(EV.LastnameTH,'') as Validatorfullname ,EP.userid as Approveruserid,isnull(EP.FirstnameTH,'') + ' ' +  isnull(EP.LastnameTH,'') as Approverfullname  from  Sys_HR_Empdetail E Inner join Sys_Master_Organizeuser ou on e.userid = ou.Userid and ou.isdelete = 0 inner join Sys_Master_Organize o on ou.Orgid = o.id  and ou.isdelete = 0 left join[Sys_Master_ApproveTQF] AP on e.userid = AP.Userid and AP.isdelete = 0 left join[Sys_HR_Empdetail] EV on EV.userid = AP.Validatoruserid left join[Sys_HR_Empdetail] EP on EP.userid = AP.Approveruserid  Where Ou.Orgid in (" + ClsEngine.Serial(Arrres) + ")";
            Dt = cn.Select(sqlcmd);
            sqlcmd = "Select isnull(e.InsRegular,'0'),isnull(e.InsTheory,'0'),isnull(e.InsExtra,'0'), u.id,isnull(FirstnameTH,'') + ' ' + isnull(LastnameTH,'') as fullname from sys_core_user u inner join Sys_HR_Empdetail e on u.id = e.userid where u.isdelete =0  and e.isdelete = 0  and (isnull(e.InsRegular,'0') <> '0' OR isnull(e.InsTheory,'0') <> '0' OR isnull(e.InsExtra,'0') <> '0')";
            Dtuser = cn.Select(sqlcmd);

            foreach (DataRow dr in Dt.Rows)
            {
                Obj = new ClsApproveTQF();
