...
{
    public partial class OfficialVisit : System.Web.UI.Page
    {
        ...
        public static string Savecost(string json)
        {
            ...
            sqlcmd = " Update [Sys_HR_OfficialVisitCost] Set IsDelete=1,DeleteDate='" + DateTime.Now + "',DeleteBy='" + ((Clsuser)HttpContext.Current.Session["My"]).userid + "' Where ovid ='" + ClsEngine.FindValue(Dicts, "Hdofficialvisitid") + "'";
            Arrcmd.Add(sqlcmd);
            ...
            foreach (DataRow dr in Dtcost.Rows)
            {
                foreach (ClsDict Objdict in Dicts)
                {
                    if (Objdict.Name.Replace("Txtcostvalue_", "") == dr["id"].ToString())
                    {
                        ...
                        sqlcmd = " INSERT INTO [Sys_HR_OfficialVisitCost] ";
                        sqlcmd += " ([id] ";
                        sqlcmd += " ,[ovid] ";
                        sqlcmd += " ,[costid] ";
                        sqlcmd += " ,[Costvalue] ";
                        ...
                        sqlcmd += " Values (";
                        sqlcmd += "'" + id + "'";
                        sqlcmd += ",'" + ClsEngine.FindValue(Dicts, "Hdofficialvisitid") + "'";
        ...
        public static List<ClsCost> Detail(string json)
        {
            ...
            ClsCost Obj;
            SqlConnector cn = new SqlConnector(Connectionstring, null);
            string sqlcmd = "Select c.id as costid, Costname,isnull(Costvalue,'') as Costvalue from Sys_Master_Cost c left join [Sys_HR_OfficialVisitCost] ovc on c.id = ovc.costid and ovc.isdelete = 0  and ovc.ovid = '" + json + "'";
        ...
        public static string Del(string json)
        {
            ...
            string id = json;
            SqlConnector Cn = new SqlConnector(Connectionstring, null);
            sqlcmd = "Update [Sys_HR_OfficialVisit] Set IsDelete=1,DeleteDate='" + DateTime.Now + "',DeleteBy='" + ((Clsuser)HttpContext.Current.Session["My"]).userid + "' Where id ='" + id + "'";
        ...
        public static string Save(string json)
        {
            ...
            string sqlcmd = "";
            List<ClsDict> Dicts = new List<ClsDict>();
            string id = ClsEngine.GenerateRunningId(Connectionstring, "Sys_HR_OfficialVisit", "id");
            //CbProvinceid:| CbCountryid:| CbProjectownerorgid:| CbDeveloptypeid:| CbOfficialvisittypeid:| 
            //CbVehicletypeid:| Txtattachment:| TxtDocno:| TxtDocumentdate:| TxtRequestfullname:ผู้อบรม #1
            //|TxtOffcialvisitname:|TxtWorkstart:|TxtWorkend:|TxtTravelstart:|TxtTravelend:
            //|TxtPlace:|TxtProjectowner:|Txtattachmentcont:|
            Dicts = ClsEngine.DeSerialized(json, ':', '|');
            sqlcmd = "  INSERT INTO [Sys_HR_OfficialVisit] ";
            sqlcmd += " ([id]";
            sqlcmd += " ,[Docno]";
            ...
            sqlcmd += ",'" + ClsEngine.FindValue(Dicts, "TxtOffcialvisitname") + "'";
            sqlcmd += ",'" + ((Clsuser)HttpContext.Current.Session["My"]).userid + "'";
            ...
            sqlcmd += ",'" + ClsEngine.FindValue(Dicts, "CbOfficialvisittypeid") + "'";
        ...
        [WebMethod]
        public static List<ClsDict> Getofficialvisittype(string json)
        {
            SqlConnector cn = new SqlConnector(Connectionstring, null);
            string sqlcmd = "Select id,officialvisittypename from Sys_Master_officialvisittype where isdelete = 0 order by officialvisittypename";
            DataTable Dt = new DataTable();
            Dt = cn.Select(sqlcmd);
            ClsDict Objdict;
            List<ClsDict> Dicts = new List<ClsDict>();
            ...
            {
                Objdict = new ClsDict();
                Objdict.Name = dr["officialvisittypename"].ToString();
                Objdict.Val = dr["id"].ToString();
                Dicts.Add(Objdict);
            }
            return Dicts;
        ...
        public static ClsGridResponse Bind(string Ctrl, long PagePerItem, long CurrentPage, string SortName, string Order, string Column, string Data, string Initial, string SelectCat, string SearchMsg, string EditButton, string DeleteButton, string Panel, string FullRowSelect, string Multiselect, string Criteria, string SearchesDat, string Searchcolumns, string WPanel, string HPanel)
        {
            ...
            if (Ctrl == "Gvofficialvisit")
            {
                PK = "id";
                Sqlcmd = "Select id, docno,convert(nvarchar,isnull(documentdate,''),103) as documentdate,offcialvisitname,place, convert(nvarchar,isnull(WorkStart,''),103) + ' - ' + convert(nvarchar,isnull(WorkEnd,''),103) as Duration,'' as Detail,'' as del,'' as [print] from Sys_HR_OfficialVisit where isdelete = 0 ";
                Criteria = "";
                SqlConnector Cn = new SqlConnector(Connectionstring, null);
                try
                {
                    ...
                    {
                        id = ((DataTable)HttpContext.Current.Session["RAW_Gvofficialvisit"]).Rows[i]["id"].ToString();
