...
{
    public partial class ApproveSupervision : System.Web.UI.Page
    {
        ...
        public static string ChangeValidate(string json)
        {
            ...
            if (cn.Select("Select * from Sys_Master_ApproveSupervision where isdelete = 0 and userid = '" + ClsEngine.FindValue(Dicts, "userid") + "'").Rows.Count == 0)
            {
                string id = ClsEngine.GenerateRunningId(Connectionstring, "Sys_Master_ApproveSupervision", "id");
                //insert

                sqlcmd = " INSERT INTO [Sys_Master_ApproveSupervision] ";
                sqlcmd += " ([id] ";
                sqlcmd += " ,[userid] ";
            ...
            {
                sqlcmd = "Update Sys_Master_ApproveSupervision set ";
        ...
        [WebMethod]
        public static List<ClsApproveSupervision> GetApproverSupervision(string json)
        {
            //json += 'CbAPOrg:' + $('#CbAPOrg').val() + '|';
            List<ClsDict> Dicts = new List<ClsDict>();
            List<ClsApproveSupervision> Objs = new List<ClsApproveSupervision>();
            SqlConnector cn = new SqlConnector(Connectionstring, null);
            Dicts = ClsEngine.DeSerialized(json, ':', '|');
            ClsApproveSupervision Obj;
            string sqlcmd = "";
            DataTable Dt = new DataTable();
            ...
            sqlcmd += " left join[Sys_Master_ApproveSupervision] AP on e.userid = AP.Userid and AP.isdelete = 0";
