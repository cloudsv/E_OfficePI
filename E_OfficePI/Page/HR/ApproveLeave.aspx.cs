...
namespace E_OfficePI.Page.HR
{
    public partial class ApproveLeave : System.Web.UI.Page
    {
        ...
        public static string ChangeValidate(string json)
        {
            ...
            if (cn.Select("Select * from Sys_Master_ApproveLeave where isdelete = 0 and userid = '" + ClsEngine.FindValue(Dicts, "userid") + "'").Rows.Count == 0)
            {
                string id = ClsEngine.GenerateRunningId(Connectionstring, "Sys_Master_ApproveLeave", "id");
                //insert

                sqlcmd = " INSERT INTO [Sys_Master_ApproveLeave] ";
                sqlcmd += " ([id] ";
                sqlcmd += " ,[userid] ";
            ...
            {
                sqlcmd = "Update Sys_Master_ApproveLeave set ";
        ...
        [WebMethod]
        public static List<ClsApproveLeave> GetApproverleave(string json)
        {
            //json += 'CbAPOrg:' + $('#CbAPOrg').val() + '|';
            List<ClsDict> Dicts = new List<ClsDict>();
            List<ClsApproveLeave> Objs = new List<ClsApproveLeave>();
            SqlConnector cn = new SqlConnector(Connectionstring, null);
            Dicts = ClsEngine.DeSerialized(json, ':', '|');
            ClsApproveLeave Obj;
            string sqlcmd = "";
            ...
            sqlcmd += " left join[Sys_Master_ApproveLeave] AP on e.userid = AP.Userid and AP.isdelete = 0";
